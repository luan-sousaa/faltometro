...
document.addEventListener("DOMContentLoaded", function () {
  ...
  function validateForm(formData) {
    ...
    if (!isValidMatricula(matricula)) {
      showMessage(
        "A matrícula deve conter apenas letras e números, com pelo menos 3 caracteres.",
        "error"
      );
      return false;
    }
    ...
    try {
      ...
      if (response.ok && data.success) {
        ...
        if (response.status === 409) {
          showMessage(
            "Email ou matrícula já cadastrados. Tente fazer login ou use dados diferentes.",
            "error"
          );
        } else {
          showMessage(
  ...
  cadastroForm.addEventListener("submit", function (e) {
    ...
    const formData = {
      ...
      tipoUsuario: document.querySelector('input[name="tipoUsuario"]:checked')
        ?.value,
